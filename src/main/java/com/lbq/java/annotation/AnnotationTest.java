 ...
 * 1.理解Annotation：
 * 		1.1 jdk5.0新增的功能
 * 		1.2 Annotation其实就是代码里的特殊标记，这些标记可以在编译，类加载，运行时被读取，并执行相应的处理。通过使用Annotation，程序员可以在不改变原有逻辑的情况下，在源文件中嵌入一些补充信息。
 ...
 * 		@deprecated:用于表示所有修饰的元素(类，方法等)已过时。通常是因为所修饰的结构危险或存在更好的选择
 ...
 *  	************出现频率较低*************
 *  	Documented：表示所修饰的Annotation在被javadoc解析时，保留下来。
